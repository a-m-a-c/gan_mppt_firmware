...
void control_init(control_config_t *config) {
  ...
  // Initialize control interface with desired config
}

void control_service(void) {
  // Update background control functionality
}

void control_set_duty(uint32_t channel, uint16_t duty_cycle) {
  // Apply a duty cycle to a channel, will apply limiters and protections
  pwm_set_duty_cycle(channel, duty_cycle);
}

void control_stop(void) {
  // Stop control interface, called on mode exit
